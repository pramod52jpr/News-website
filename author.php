        ...
        <div class="news-container">
            <div class="news">
                <?php
                $sql4="select * from user where `User-Id`=$aid";
                $result4=mysqli_query($conn,$sql4);
                $row4=mysqli_fetch_assoc($result4); ?>
                <h1><?php echo 'News By '.$row4['First-Name'].' '.$row4['Last-Name'] ?></h1>
                ...
                <div class="pagination">
                    <?php if($page_index>1){ ?>
                        <a style="border-radius:8px 0px 0px 8px" href="<?php echo $_SERVER['PHP_SELF'].'?page='.($page_index-1).'&aid='.$aid; ?>">Prev</a>
                    <?php } ?>
                    <?php for($i=1;$i<=$page;$i++){ 
                        ...
                        echo "<a$active href='$_SERVER[PHP_SELF]?page=$i&aid=$aid'>$i</a>";
                    } ?>
                    <?php if($page_index<$page){ ?>
                        <a style="border-radius:0px 8px 8px 0px" href="<?php echo $_SERVER['PHP_SELF'].'?page='.($page_index+1).'&aid='.$aid; ?>">Next</a>
                    <?php } ?>
                </div>
            </div>
            <div class="sidebar">
